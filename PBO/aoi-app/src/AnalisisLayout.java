...
public class AnalisisLayout extends JFrame {
    ...
    private void initUI() {
        setTitle("Detail Proyek: RNV-JKT-AXG-001-Aoi-Midori");
        ...
        JLabel title = new JLabel("Detail Proyek: RNV-JKT-AXG-001-Aoi-Midori");
        title.setFont(title.getFont().deriveFont(Font.BOLD, 16f));
        header.add(title, BorderLayout.WEST);
        main.add(header, "spanx, wrap, growx");


        main.add(createClientPanel(), "grow, align center");
    ...
    private JPanel createClientPanel() {
        ...
        JPanel contentPanel = new JPanel(new MigLayout("insets 8", "[grow][grow]", "[]4[]4[]4[]8[]"));
    ...
    private void showDatePickerDialog(JTextField targetField, JLabel lastEditDateLabel, JLabel lastEditedByLabel) {
    String newDate = JOptionPane.showInputDialog(null, "Enter new date (yyyy-MM-dd):");

    if (newDate != null && !newDate.trim().isEmpty()) {
        targetField.setText(newDate);

        lastEditDateLabel.setText(LocalDate.now().toString());

        lastEditedByLabel.setText("CurrentUser");
    }
}
    ...
    editEst.addActionListener(e -> {
        showDatePickerDialog(estDateField, lastEditValue, lastEditedByValue);
    });
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(() -> {
            AnalisisLayout ex = new AnalisisLayout();
            ex.setVisible(true);
        });
    }
}